...
function CheckoutContent() {
      ...
      <div className="max-w-2xl mx-auto px-4 py-16 text-center space-y-6">
        ...
        <div className="flex justify-center gap-4">
          <button
            ...
            className="px-6 py-3 rounded-xl bg-zinc-950 text-white font-bold text-xs hover:bg-zinc-800 shadow-xs transition-all tactile-btn"
          >
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className={`rounded-3xl p-5 border shadow-2xs transition-all ${
        isTimerWarning
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className={`text-sm font-extrabold ${
                isTimerWarning ? 'text-red-800' : 'text-zinc-950'
              }`}>
                {isTimerWarning ? 'Waktu Hampir Habis!' : 'Waktu Checkout Sesi'}
              </h2>
              <p className={`text-[10px] font-medium ${
                isTimerWarning ? 'text-red-600' : 'text-zinc-500'
              }`}>
                Selesaikan pembayaran sebelum batas waktu reservasi tiket habis
              </p>
            </div>
          </div>
          <div className={`text-3xl font-black font-mono ${
            isTimerWarning ? 'text-red-600' : 'text-zinc-950'
          }`}>
            {timerText}
          </div>
        </div>
        {/* Progress Bar */}
        <div className="mt-3 w-full h-2 bg-zinc-100 rounded-full overflow-hidden border border-zinc-200">
          <div
            className={`h-full rounded-full transition-all duration-1000 ${
              isTimerWarning ? 'bg-red-500' : 'bg-zinc-950'
            }`}
                    ...
                    >
                      <div className="space-y-2 flex-1">
                        <div className="flex items-center gap-2.5">
                          <span
                            className="w-3.5 h-3.5 rounded-full shrink-0 border border-zinc-300 shadow-2xs"
                            style={{ backgroundColor: tier.color }}
                          />
                          <h3 className="font-extrabold text-zinc-950 text-base">{tier.name}</h3>
                          <span
                        ...
                        {tier.description && (
                          <p className="text-xs text-zinc-600 font-normal leading-relaxed">{tier.description}</p>
                        )}
                        <div className="text-base font-black text-zinc-950 font-mono">
                          Rp {tier.price.toLocaleString('id-ID')}{' '}
                          <span className="text-xs font-normal text-zinc-400">/ tiket</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 shrink-0 self-end md:self-center">
                        {isSoldOut ? (
                          <span className="text-xs font-bold text-zinc-400 bg-zinc-100 border border-zinc-200 px-4 py-2 rounded-xl font-mono">
                            Sold Out
                          </span>
                        ) : (
                          <div className="flex items-center gap-2 bg-zinc-100 p-1.5 rounded-2xl border border-zinc-200">
                            <button
                              ...
                              className="w-8 h-8 rounded-xl bg-white text-zinc-700 font-bold hover:bg-zinc-200 disabled:opacity-30 transition-all flex items-center justify-center shadow-2xs tactile-btn"
                            >
                              <Minus className="w-3.5 h-3.5" />
                            </button>
                            <span className="w-8 text-center text-sm font-extrabold text-zinc-950 font-mono">
                              {currentQty}
                            </span>
                            <button
                              onClick={() => handleQuantityChange(tier, 1)}
                              disabled={currentQty >= available}
                              className="w-8 h-8 rounded-xl bg-zinc-950 text-white font-bold hover:bg-zinc-800 disabled:opacity-30 transition-all flex items-center justify-center shadow-2xs tactile-btn"
                            >
        ...
        <div className="md:col-span-1 space-y-4">
          <div className="bg-white rounded-3xl border border-slate-200 p-6 space-y-4 shadow-xs">
            ...
            <div className="space-y-2 pt-1 border-b border-slate-100 pb-4">
              ...
              {appliedPromo ? (
                <div className="p-3 bg-zinc-50 border border-zinc-200 rounded-2xl flex items-center justify-between shadow-2xs">
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-xl bg-zinc-950 text-white flex items-center justify-center font-bold text-xs font-mono">
                      %
                    </div>
                    <div>
                      <div className="font-bold text-xs text-zinc-950 font-mono">
                  ...
                  <button
                    ...
                    className="text-zinc-400 hover:text-red-600 p-1 rounded-lg hover:bg-zinc-100 transition tactile-btn"
                    title="Hapus Promo"
                  >
                    <X className="w-4 h-4" />
                  </button>
              ...
              {promoMessage && (
                <p
                  className={`text-[11px] font-medium mt-1 ${
                    promoMessage.type === 'success' ? 'text-emerald-700 font-bold' : 'text-red-700 font-bold'
                  }`}
                >
                  {promoMessage.text}
                </p>
            ...
            <div className="space-y-2 text-xs">
              ...
              {discountAmount > 0 && (
                <div className="flex justify-between items-center text-emerald-700 font-semibold font-mono">
              ...
              <div className="flex justify-between items-center text-sm pt-2 border-t border-zinc-100 font-bold text-zinc-950">
                <span>Total Tagihan</span>
                <span className="font-black text-zinc-950 text-lg font-mono">
...
export default function CheckoutPage() {
  return (
    <Suspense
      fallback={
        <div className="max-w-4xl mx-auto px-4 py-20 text-center text-zinc-400 text-xs animate-pulse flex items-center justify-center gap-2">
