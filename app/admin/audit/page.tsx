  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <button
          onClick={fetchLogs}
          className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-slate-200 text-xs font-bold text-slate-600 hover:bg-slate-50 transition shadow-xs"
        >
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-indigo-500 placeholder:text-slate-400 bg-white"
        />
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
        <div className="divide-y divide-slate-100">
          {filtered.map((log) => (
            <div key={log.id} className="p-5 hover:bg-slate-50 transition space-y-2">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black bg-slate-900 text-white font-mono tracking-wider">
                    {log.action}
                  </span>
                  {log.target_id && (
              ...
              <div className="flex items-center gap-2 text-[11px] text-slate-500 font-medium">
                ...
                <span>Pelaksana: <span className="font-semibold text-slate-700">{log.actor_email}</span> ({log.actor_role})</span>
              </div>
            </div>
          ))}
