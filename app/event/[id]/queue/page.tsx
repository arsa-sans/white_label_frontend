...
export default function QueuePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-16 w-full max-w-full overflow-x-hidden bg-[#F8F9FA]">
      <div className="w-full max-w-md text-center space-y-8">
        {/* Event Context */}
        {eventBanner && (
          <div className="relative w-24 h-24 mx-auto rounded-3xl overflow-hidden shadow-2xs border border-zinc-200">
            <img src={eventBanner} alt={eventName} className="w-full h-full object-cover" />
          </div>
        )}
        {eventName && (
          <p className="text-xs font-extrabold text-zinc-500 uppercase tracking-wider">{eventName}</p>
        )}
        ...
        <div className="relative">
          <div className="relative w-32 h-32 mx-auto">
            <div className={`absolute inset-0 rounded-full border-4 ${
              admitted ? 'border-emerald-400' : 'border-zinc-200'
            } animate-ping opacity-20`} />
            <div className={`absolute inset-2 rounded-full border-4 ${
              admitted ? 'border-emerald-500' : 'border-zinc-300'
            } animate-pulse opacity-40`} />
            <div className={`absolute inset-4 rounded-full ${
              admitted ? 'bg-emerald-50 border border-emerald-200' : 'bg-white border border-zinc-200'
        ...
        {!admitted && (
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-white rounded-3xl border border-zinc-200 p-4 shadow-2xs">
        ...
        {/* Info Note */}
        <div className="bg-white rounded-2xl border border-zinc-200 p-4 text-xs text-zinc-600 font-medium shadow-2xs flex items-center justify-center gap-2">
